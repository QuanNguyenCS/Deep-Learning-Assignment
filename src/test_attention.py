...
# 3-Layer MLP with Input Feature Attention
class MLPWithAttention(torch.nn.Module):
    def __init__(self, input_dim=512, hidden_dim1=256, hidden_dim2=128, output_dim=40, reduction=8):
        super().__init__()
        self.attention = FeatureAttention(dim=input_dim, reduction=reduction)
        self.net = torch.nn.Sequential(
            torch.nn.Linear(input_dim, hidden_dim1),
            torch.nn.LayerNorm(hidden_dim1),
            torch.nn.ReLU(),
...
# 3-Layer MLP with Multi-Layer Feature Attention (Input + First Hidden Layer)
class MLPMultiLayerAttention(torch.nn.Module):
    def __init__(self, input_dim=512, hidden_dim1=256, hidden_dim2=128, output_dim=40, reduction=8):
        super().__init__()
        self.attention1 = FeatureAttention(dim=input_dim, reduction=reduction)
        self.layer1 = torch.nn.Linear(input_dim, hidden_dim1)
        self.ln1 = torch.nn.LayerNorm(hidden_dim1)
        self.relu1 = torch.nn.ReLU()
        
        self.attention2 = FeatureAttention(dim=hidden_dim1, reduction=reduction)
        self.dropout1 = torch.nn.Dropout(0.2)
        
        self.net_rest = torch.nn.Sequential(
            torch.nn.Linear(hidden_dim1, hidden_dim2),
            torch.nn.LayerNorm(hidden_dim2),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.2),
    ...
    def forward(self, x):
        ...
        # Attention at first hidden layer
        x = self.attention2(x)
        x = self.dropout1(x)
        
        # Rest of network
        return self.net_rest(x)

def main():
    print("=" * 80)
    print("Feature Attention Evaluation: Baseline MLP vs Attention MLP")
    print("=" * 80)
    ...
    attn_checkpoints_root = checkpoints_root / "attention"
    attn_checkpoints_root.mkdir(parents=True, exist_ok=True)
    ...
    # Models to compare
    models_to_test = [
        {"name": "Baseline MLP (No Attention)", "class": MLPClassifier, "kwargs": {}},
        {"name": "Attention MLP (SE Gating)", "class": MLPWithAttention, "kwargs": {"reduction": 8}},
        {"name": "Multi-Layer Attention MLP", "class": MLPMultiLayerAttention, "kwargs": {"reduction": 8}}
    ]
    ...
    for model_cfg in models_to_test:
        ...
        save_path = attn_checkpoints_root / save_name
    ...
    print("\n" + "=" * 90)
    print("FEATURE ATTENTION LEADERBOARD (sorted by Test Acc)")
    print("=" * 90)
    print(f"{'Model Configuration':<30} | {'Best Val Loss':<15} | {'Test Accuracy':<15} | {'Train Time':<10}")
    print("-" * 90)
    for res in results:
        print(f"{res['name']:<30} | {res['val_loss']:<15.4f} | {res['test_acc']:<15.4%} | {res['time']:<10.1f}s")
    print("=" * 90 + "\n")
